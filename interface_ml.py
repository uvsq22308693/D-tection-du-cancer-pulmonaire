...
with st.form(key='tabulaire_form'):
    ...
    # --- correction ici : nombre et non radio ---
    toux_chronique = st.number_input("Taux de toux chronique", min_value=0.0, max_value=100.0, step=0.1)
...
if submit_button:
    if uploaded_file is None:
        ...
        #st.image(image, caption="Image du nodule", use_column_width=True)
        st.image(uploaded_file, width=700)  # ou la largeur que tu veux
        ...
        response = requests.post(api_url, params=params, files={"file": uploaded_file.getvalue()})
